...
| `index.ndx` | Index file | ✅ (created by gmx select) |
...
| `-ff amber14sb` | `config.sh`: `FORCEFIELD="amber99sb-ildn"` | Use system-installed force field (avoids partial-FF DNA/ion issues) |
...
| `gmx make_ndx` with groups 1,12,16,17 | `lib/stages.sh` `run_stage_index()` | Uses `gmx select` (`not (group Water or group Ion)` / `group Water or group Ion`), no group numbers |
