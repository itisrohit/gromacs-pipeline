...
# ── Index file creation ──
# Uses GROMACS' default index groups (auto-generated by molecule type)
# and combines them into the groups expected by the MDP temperature
# coupling lines (tc-grps = Protein_DNA Water_Ions).
#
# GROMACS auto-assigns groups in a consistent order:
#   1   = Protein (if present)
#   12  = DNA (if present, typically)
#   16  = Ion (all ions combined)
#   17  = Water (SOL)
#
# This function starts from the default groups (keep 0) and combines
# them by their standard numbers. The groups must match the MDP tc-grps.
run_stage_index() {
...
EOF
    ...
    if ! grep -q "Protein_DNA" "$out" 2>/dev/null; then
        ...
        echo "         Temperature coupling will use available groups."
    fi
    echo "INDEX: Complete"
}
